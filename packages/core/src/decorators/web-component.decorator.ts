...
import { ClassDecorator } from "@xendar/common";
import { WebComponentConfig } from "../models/web-component-config.type";
 ...
 */
export function WebComponent<T extends BaseWebComponent>(config: WebComponentConfig): ClassDecorator<T> {
  return function (klass: Constructor<T>, _context: ClassDecoratorContext): void {
    setObservedAttributes(klass, config.attributes);
    setSelectors(klass, config.selectors);
  };
}

/**
 * Function to set the observedAttributes static property on the class.
 ...
 */
function setObservedAttributes<T extends BaseWebComponent>(klass: Constructor<T>, attributes: string[] | undefined): void {

          ...
          switch (previousState) {
            case LexerState.ATTRIBUTE:
              if (cursor.peek() !== DOUBLE_QUOTE) {
                throw `Attribute interpolation expression must end with double quotes '"', found '${String.fromCharCode(cursor.peek())}' at ${cursor.formattedPosition}`;
              }
              // Consume '"'
              cursor.advance();
            ...
            default:
              throw `Unexpected state '${previousState}' after interpolation literal at ${cursor.formattedPosition}`;
          };
